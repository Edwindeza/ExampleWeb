...
class SendMail extends PHPMailer
{
    ...
    public function html_view_mail($arrMail){
        ...
        for($f=0;$f<count($arrMail['head-table']);$f++){
            $headTable .= '<th style="border: 1px solid black;border-collapse: collapse;">'.htmlspecialchars($arrMail['head-table'][$f]).'</th>'; 
        }
        
        for($i=0;$i<count($arrMail['body-table']);$i++){
              $this->viewMailData .= '<td style="border: 1px solid black;border-collapse: collapse;">'.htmlspecialchars($arrMail['body-table'][$i]).'</td>';
        }
        
        $this->htmlMail = file_get_contents($arrMail['url']);
